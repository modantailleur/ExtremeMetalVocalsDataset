...
with h5py.File(out_path + out_name+ '.h5', 'r') as hf:
    for split_id in range(n_splits):
        inference_dataset = hf[f'Singer{split_id+1}']['inference']['inference']
        groundtruth_dataset = hf[f'Singer{split_id+1}']['inference']['groundtruth']
        name += [item.decode('utf-8') for item in hf[f'Singer{split_id+1}']['inference']['audio_name']]
