...
class RegularSplitDataset(object):
    def __init__(self, hdf5_path, split_id, split_type='train'):
        ...
        with h5py.File(self.hdf5_path, 'r') as hf:
            gt = hf[f'split_k_{split_id}'][split_type]['groundtruth'][:]
            ...
            self.audio_names = [audio_name for audio_name in hf[f'split_k_{split_id}'][split_type]['audio_name'][:]]
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(self.hdf5_path, 'r') as hf:
            audio_name =  hf[f'split_k_{self.split_id}'][self.split_type]['audio_name'][idx].decode('utf-8')
            mel_spectrogram =  hf[f'split_k_{self.split_id}'][self.split_type]['mel_spectrogram'][idx]
            groundtruth =  hf[f'split_k_{self.split_id}'][self.split_type]['groundtruth'][idx]
...
class SingerClassificationDataset(object):
    def __init__(self, hdf5_path, split_id, split_type='train'):
        ...
        with h5py.File(self.hdf5_path, 'r') as hf:
            gt = hf[f'split_k_{split_id}'][split_type]['singer'][:]
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(self.hdf5_path, 'r') as hf:
            audio_name =  hf[f'split_k_{self.split_id}'][self.split_type]['audio_name'][idx].decode('utf-8')
            mel_spectrogram =  hf[f'split_k_{self.split_id}'][self.split_type]['mel_spectrogram'][idx]
            groundtruth =  hf[f'split_k_{self.split_id}'][self.split_type]['singer'][idx]
            # groundtruth = groundtruth.decode('utf-8')
            groundtruth = self.gt_str_to_onehot[groundtruth]
            # print('CCCCCC')
            # print(groundtruth)
...
class TechniqueClassificationDataset(object):
    def __init__(self, hdf5_path, split_id, split_type='train'):
        ...
        with h5py.File(self.hdf5_path, 'r') as hf:
            gt = hf[f'split_k_{split_id}'][split_type]['technique'][:]
            gt = np.array([item.decode('utf-8') for item in gt])
            class_sample_count = np.array(
                        [len(np.where(gt == t)[0]) for t in classes_dict.keys()])
            weight = 1. / class_sample_count
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(self.hdf5_path, 'r') as hf:
            audio_name =  hf[f'split_k_{self.split_id}'][self.split_type]['audio_name'][idx].decode('utf-8')
            mel_spectrogram =  hf[f'split_k_{self.split_id}'][self.split_type]['mel_spectrogram'][idx]
            groundtruth =  hf[f'split_k_{self.split_id}'][self.split_type]['technique'][idx]
            groundtruth = groundtruth.decode('utf-8')
            groundtruth = self.gt_str_to_onehot[groundtruth]
