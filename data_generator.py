...
class SingerClassificationDataset(object):
    def __init__(self, hdf5_path, split_id, split_type='train'):
        """This loader is used for singer classification (find the singer from Mel spectrogram)

        Args:
        - hdf5_path: path where to find the mel dataset
        - split_id: id of the cross-validation split
        - split_type: type of split (among 'train', 'valid', 'eval')
        """
...
class TechniqueClassificationDataset(object):
    def __init__(self, hdf5_path, split_id, split_type='train'):
        """This loader is used for multi-class technique classification (find the technique from Mel spectrogram)
        ...
        with h5py.File(self.hdf5_path, 'r') as hf:
            ...
            #tried weighting the classes depending on their occurence in the dataset, but it leads to better accuracy if we don't
...
class BinaryTechniqueClassificationDataset(object):
    """This loader is used for binary technique classification (find distorsion or clear voice from Mel spectrogram)
