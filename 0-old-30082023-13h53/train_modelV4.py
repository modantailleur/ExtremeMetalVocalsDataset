...
for split_id in range(n_splits):
    ...
    valid_datasets.append(SingerClassificationDataset(hdf5_path=data_path+dataset_name+'.h5', split_id=split_id, split_type='valid'))
    eval_datasets.append(SingerClassificationDataset(hdf5_path=data_path+dataset_name+'.h5', split_id=split_id, split_type='eval'))

for split_id in range(n_splits):
    trainer = MelRegularTrainer(model=models[split_id], models_path='./model/', model_name=f'model_singer_classif_no_distorsion_{split_id+1}', split_id=split_id, train_dataset=train_datasets[split_id], valid_dataset=valid_datasets[split_id], eval_dataset=eval_datasets[split_id])
