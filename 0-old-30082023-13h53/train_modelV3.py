...
for split_id in range(n_splits):
    trainer = MelRegularTrainer(model=models[split_id], models_path='./model/', model_name=f'model_singer_classif{split_id+1}', split_id=split_id, train_dataset=train_datasets[split_id], valid_dataset=valid_datasets[split_id], eval_dataset=eval_datasets[split_id])
