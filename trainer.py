...
class MelTrainer:
    def __init__(self, model, models_path, model_name, split_id, train_dataset=None, valid_dataset=None,
                 eval_dataset=None, out_path='./outputs/', out_name='dataset_inference', learning_rate=1e-3, dtype=torch.FloatTensor):
        """
        Initializes the MelTrainer class. This class trains and evaluates a model only on Mel spectrogram values.

        Args:
        - model: The model architecture.
        - models_path: The path to save the trained model.
        - model_name: The name of the model.
        - split_id: the id of the cross-validation split.
        - train_dataset: The training dataset.
        - valid_dataset: The validation dataset.
        - eval_dataset: The evaluation dataset.
        - out_path: the output path where the outputs of the model after inference are stored
        - out_name: the name of the output h5 file
    ...
    def validate(self, dataset, cur_epoch, batch_size=64, device=torch.device("cpu"), label='VALIDATION', forced=False):
        ...
        # in case weighted classes are used in the data loader
